...
def display():
    global cameraIndex, cow2wld , CowNum, cow2wldList, cowLocationList ,cursorOnCowBoundingBox, B_spline, RollerCosterStart, RotatingTime, MaxCowNum
    ...
    if CowNum < MaxCowNum:
        ...
        if(RollerCosterTime >= RotatingTime):
            ...
            Rotate = np.array([t**3,t**2,t,1]) @ B_spline @ Pose

            cow = cow2wldList[(int(RollerCosterTime))%6].copy()
            cow[:3:,3] = Rotate
            
            DirectVector = np.array([3*t*t, 2*t, 1, 0]) @ B_spline @ Pose
        ...
        if state == GLFW_DOWN:
            ...
            if isDrag==V_DRAG and cursorOnCowBoundingBox is True:
                ...
                if CowNum == MaxCowNum:
                    ...
                    isDrag = 0
            print( "Left mouse up\n");
            # start horizontal dragging using mouse-move events.s
...
def main():
    ...
    width = 800;
    height = 600;
    window = glfw.create_window(width, height, '2018008922-PA02', None, None)
    if not window:
        glfw.terminate()
        sys.exit(-1)
